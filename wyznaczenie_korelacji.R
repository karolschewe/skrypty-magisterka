...
meansq.KONSERWA<-df.mean$mean.party[df.mean$przyporzadkowanie=="KONSERWATYZM"]^2
meansq.RESZTA<-df.mean$mean.party[df.mean$przyporzadkowanie=="ANTYKONSERWATYZM"]^2
...
calculate.one.correlation<-function(terytek,threshold.km){
...
calculate.one.correlation(TERYT.list[8],30)
odleglosci<-c(10,15,20,25,30,40,50,75,100,150,200,250,300,400,500)
for (km in odleglosci)
{
  tic(paste0(km,"km"))
  assign(paste0("corr.",km),lapply(TERYT.list, calculate.one.correlation, km))
